...
def process_blog_post(url):
    ...
    with requests.Session() as session:
        ...
        try:
            ...
            # Remove unwanted elements (e.g., footer, forms, editorial info, share buttons)
            for element in soup.find_all(['footer', 'form', 'script', 'style', 'div', 'span']):
                # Remove editorial information
                if element.get_text(strip=True).startswith("By ") and "|" in element.get_text():
                    element.decompose()
                # Remove share buttons
                if "share" in element.get('class', []) or "shares" in element.get_text().lower():
                    element.decompose()
                # Remove specific elements by class or ID
                if element.get('class') and any(cls in element.get('class') for cls in ['editorial', 'disclosure', 'social-share']):
                    element.decompose()
                if element.get('id') and any(id in element.get('id') for id in ['share-buttons', 'editorial-info']):
                    element.decompose()
            debug_log("Removed unwanted elements (footer, forms, editorial info, share buttons, etc.)")

            # Extract title
            title_element = soup.find('h1') or soup.title
            original_title = title_element.get_text(strip=True) if title_element else "Untitled Post"
            debug_log(f"Original title: {original_title}")
            ...
            content_element = soup.find('article') or soup.body
            raw_content = str(content_element) if content_element else ''
